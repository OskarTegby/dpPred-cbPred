...
TLB::allocate(IntPtr address, SubsecondTime now)
{
   ...
   if (give_size() == phist_size && hitCounter[temp_hash_vpn][temp_hash_pc] > 6) {
        ++m_bypass;
        shadow_table_insert (temp_vpn);
	addRecentPFN(address & 0xfffffffffffff000);
        return;
   } else if (give_size() == phist_size) {
   ...
   if (eviction && give_size() == phist_size) {
